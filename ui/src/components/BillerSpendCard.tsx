    ...
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> {t('billerSpend.in')}
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> {t('billerSpend.out')}
              {" · "}
              {row.providerCount === 1 ? t('billerSpend.providerCount', { count: row.providerCount }) : t('billerSpend.providerCount_plural', { count: row.providerCount })}
              {" · "}
              {row.modelCount === 1 ? t('billerSpend.modelCount', { count: row.modelCount }) : t('billerSpend.modelCount_plural', { count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        ...
        <div className="text-xs text-muted-foreground">
          {row.apiRunCount === 1 ? t('billerSpend.meteredRuns', { count: row.apiRunCount }) : t('billerSpend.meteredRuns_plural', { count: row.apiRunCount })}
          {" · "}
          {row.subscriptionRunCount === 1 ? t('billerSpend.subscriptionRuns', { count: row.subscriptionRunCount }) : t('billerSpend.subscriptionRuns_plural', { count: row.subscriptionRunCount })}
          {" · "}
