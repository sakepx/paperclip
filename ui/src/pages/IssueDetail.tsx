...
function ActorIdentity({ evt, agentMap, t }: { evt: ActivityEvent; agentMap: Map<string, Agent>; t: (key: string) => string }) {
...
export function IssueDetail() {
  ...
  const interruptQueuedComment = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({
        ...
        body: t('issueDetail.interruptDesc'),
        tone: "success",
      });
    },
    onError: (err) => {
        ...
        title:
          variables.sharingPreferenceAtSubmit === "prompt"
            ? variables.allowSharing
              ? t('issueDetail.feedbackSavedSharing')
              : t('issueDetail.feedbackSavedLocal')
            : variables.allowSharing
              ? t('issueDetail.feedbackSharingEnabled')
  ...
  const archiveFromInbox = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t('issueDetail.issueArchived'), tone: "success" });
    },
    onError: (err) => {
      pushToast({
