...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t('pluginSettings.failedToSave') });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t('pluginSettings.testPassed') });
      } else {
        setTestResult({ type: "error", text: result.message || t('pluginSettings.testFailed') });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t('pluginSettings.testFailed') });
    },
  });
